...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Update config settings for the training script."
    )

    parser.add_argument(
        "--modes",
        type=int,
        help="Number of fourier modes to preserve",
        default=None
    )
    ...
    parser.add_argument(
        "--tr_slice",
        type=int,
        help="The amount of training data slice to keep",
        default=None
    )
    ...
    for key, value in args_dict.items():
        if value is not None:
            hyperparameter_defaults[key] = value
